...
def __find_cluster(x, means, clusters):
    """
...
def __create_initial_means(data, k):
    """
    ...
    while index < k:
        ...
        if random_index not in selected_indices:
            ...
            means.append(data[random_index])
            index += 1
    return means
...
def get_clusters(data, k):
    ...
    means = __create_initial_means(data, k)
    ...
    while not done:
        ...
        for index in range(len(data)):
            row = data[index]
            cluster_index = __find_cluster(row, means, clusters)
